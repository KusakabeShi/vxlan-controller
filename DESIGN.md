...
### Client 端增量上報與 MACsSynced

Client 監聽鄰居表變更後，向 Controller 發送增量 MACUpdate。發送前檢查 `MACsSynced`:

* `MACsSynced=true`: 正常塞入 incremental update。若 queue 滿（`default` branch），設 `MACsSynced=false`
* `MACsSynced=false`: 表示 sendloop 接下來會發 full state（會替換 queue item 的 State）。此時仍需塞一個空的 `QueueItem{}` 喚醒 sendloop，否則若 queue 為空，sendloop 永遠不會醒來發送 full state
